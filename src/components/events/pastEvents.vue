    ...
    <v-container class="pa-0 mb-3">
        <v-layout wrap align-center justify-center row fill-height class="mt-0 mb-0" >
           <v-flex xs12 md12 lg12 class="pa-2 mb-0">
               ...
               <p class="google-font" style="font-size:110%;color:#616161 ">Here are the recent 10 meetups. To know more about the past meetups <a :href="chapterDetails.ChapterPastEventsLink" target="_blank" style="text-decoration:none;color:#0277bd">Click here</a></p>
...
export default {
    ...
    created(){
        fetch('https://gdg-cors-proxy.herokuapp.com/https://api.meetup.com/'+MeetupAPI.urlname+'/events?desc=true&photo-host=public&page=8&status=past&key='+MeetupAPI.apiKey).then(data=>data.json()).then(res=>{
